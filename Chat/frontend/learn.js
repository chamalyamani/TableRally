...
sideSearchBar.addEventListener("input", (input) => {
  ...
  singleConversationList.forEach((singleConversationList) => {
    // console.log(singleConversationList.single);
    let matchingUser = singleConversationList.single
      .querySelector(".li1")
      .textContent.toLowerCase()
      .includes(value);
    if (matchingUser) matchingUsers.push(singleConversationList);
  });
    ...
    getAccessToken()
      ...
      .then((data) => {
        ...
        if (data["status"]) {
          ...
          newSingleConversation.querySelector(".li1").textContent =
            userData.username;
          newSingleConversation.style.display = "flex";
          newSingleConversation.querySelector(".cvr-img").src =
            userData.image_url;
          let fullConv = {
          ...
          // })
          convClick(
            { id: data.id, conversation: userData.username },
...
function convClickAction(conv, singleConv) {
  ...
  if (currentConversation) currentConversation.style.backgroundColor = "";
...
function listMessages(conv) {
  getAccessToken()
    ...
    .then((response) => response.json())
    .then((data) => {
      conversationTopBar.querySelector(".top-bar-right .p1").textContent =
        data.display_name;
      mainChat.innerHTML = "";
      data.messages.forEach((message) => {
        if (message.sender == conv.conversation) {
          let newLeftMessage = leftMessage.cloneNode(true);
          newLeftMessage.querySelector(".left-message-p").textContent =
            message.content;
          ...
          newRightMessage.querySelector(".right-message-p").textContent =
            message.content;
...
async function realTime(conv, singleConv) {
  ...
  socket.onmessage = ({ data }) => {
    ...
    if (receivedMessage.type == "send_message") {
      ...
      else messages = mainChat.querySelectorAll(".right-message");
      ...
      disableMessageBar();
      removeBlur();
      if (currentUser == receivedMessage.blocked) blockButton.disabled = true;
      else {
        blockChat.style.display = "none";
        unblockChat.style.display = "block";
        div3.querySelector(
          ".modal-content3 .modal-delete-message3"
        ).textContent = "UNBLOCK THIS USER?";
      }
      ...
      div3.querySelector(".modal-content3 .modal-delete-message3").textContent =
        "BLOCK THIS USER?";
    }
      ...
      if (messageInput.value) {
        if (messageInput.value.trim() != "") {
          let jsonMessage = {
            ...
            user: singleConv.querySelector(".li1").textContent,
          };
