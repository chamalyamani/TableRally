...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        user_var = await sync_to_async(User.objects.get)(id=self.user.id)

        print("[", user_var.__dict__,"]")
        # print(self.channel_name, acepted_users)
    ...
    async def add_to_woiting_list(self):
        woiting_list.put(self.user.id)
        # await self.match_players()
        
    async def match_players(self):
        index = 0
        while True:
            if woiting_list.qsize() == 2:
                self.player1 = woiting_list.get()
                self.player2 = woiting_list.get()
                if self.user.id == self.player1:
                    self.game = LiveGame()
                    ...
                    main_player.append(self.user.id)
                ...
                try:
                    index = player_wait_for_group.index(self.user.id)
    ...
    async def GameLoop(self, index):
        while True:
            if self.user.id == player_wait_for_group[index]:
                self.game.RunGame()
                await self.channel_layer.group_send(
                self.group_name,
                {
    ...
    def create_score_entry(self):
        Score.objects.create(
            user1_id=self.player1,
            user2_id=self.player2,
            score1=self.game.score1,
            score2= self.game.score2
        )
