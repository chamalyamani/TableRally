...
// export async function showFriendCard(user, shadowRoot, friendshipId = null) {
//   const friendImage = shadowRoot.querySelector('.friend-avatar img');
//   const friendName = shadowRoot.querySelector('.friend-name');
//   const overlay = shadowRoot.querySelector('.overlay');
//   const friendCard = shadowRoot.querySelector('.friend-card');
//   const addBtn = shadowRoot.querySelector('#addFriend-btn');
//   const blockBtn = shadowRoot.querySelector('#blockFriend-btn');
//   const statusMsg = shadowRoot.querySelector('.status-msg'); // Optional status message element

//   // Update Friend Card information
//   friendImage.src = user.image_url || '/default-avatar.png'; // Fallback if no image
//   friendName.textContent = user.username || 'Unknown User'; // Fallback if no username
...
//   const blocked = await isUserBlocked(user.username);

//   if (blocked) {
//     // Hide "Add Friend" button and show "Unblock User" button
//     addBtn.style.display = 'none';
//     blockBtn.style.display = 'inline-block';
//     blockBtn.textContent = 'Unblock User';

//     // Show a status message indicating the user is blocked
//     if (statusMsg) {
//       statusMsg.textContent = 'You have blocked this user.';
//       statusMsg.style.display = 'block';
//     }
//   } else {
//     // Update the "Add Friend" button based on whether a friendshipId exists
//     if (friendshipId) {
//       addBtn.textContent = 'Cancel Request';
//       addBtn.dataset.friendshipId = friendshipId; // Store friendshipId in data attribute
//     } else {
//       addBtn.textContent = 'Add Friend';
//       delete addBtn.dataset.friendshipId;
//     }

//     // Ensure the "Add Friend" button is visible and enabled
//     addBtn.style.display = 'inline-block';
//     addBtn.disabled = false;

//     // Show the "Block Friend" button
//     blockBtn.style.display = 'inline-block';
//     blockBtn.textContent = 'Block Friend';

//     // Hide the status message if any
//     if (statusMsg) {
...
// export function hideFriendCard(shadowRoot) {
//   const overlay = shadowRoot.querySelector('.overlay');
//   const friendCard = shadowRoot.querySelector('.friend-card');
...
//   try {
//     const token = await getAccessToken(); // Ensure this function retrieves the JWT token correctly
...
//       },
//       credentials: 'include',
//       body: JSON.stringify({ to_username: username }), // Pass the username in the body
//     });

//     const data = await response.json(); // Parse the response (success or error)
...
//     // Return the friendship_id for further use
//     return data.friendship_id;
//   } catch (error) {
//     globalNotifPopup('Error', `${error.message}`); // Display the error message
//     return null;
//   }
// }
...
//     if (!response.ok) {
//       // Handle errors, possibly user is not found or no friendship exists
//       console.error('Error fetching friendship status:', data.error);
//       return null;
//     }

//     // Return the friendship ID if a pending request exists
...
//   try {
//     const token = await getAccessToken(); // Ensure this function retrieves the JWT token correctly
//     const response = await fetch(`/friends/is-blocked/${username}/`, {
//       method: 'GET',
//       headers: {
//         'Authorization': `Bearer ${token}`,
//         'Content-Type': 'application/json',
...
//     if (!response.ok) {
//       // Handle errors, possibly user is not blocked
//       console.error('Error checking block status:', data.error);
//       return false;
//     }
...
// async function handleAddCancelFriend(shadowRoot) {
//   const addBtn = shadowRoot.querySelector('#addFriend-btn');
//   const friendshipId = addBtn.dataset.friendshipId;
//   const username = shadowRoot.querySelector('.friend-name').textContent;
...
//     if (success) {
//       // Update UI to "Add Friend"
//       addBtn.textContent = 'Add Friend';
//       delete addBtn.dataset.friendshipId;
//     }
//   } else {
//     // Send a friend request
//     const newFriendshipId = await addFriend(username);
//     if (newFriendshipId) {
//       // Update UI to "Cancel Request"
//       addBtn.textContent = 'Cancel Request';
//       addBtn.dataset.friendshipId = newFriendshipId;
//     }
//   }
...
// async function handleBlockFriend(shadowRoot) {
//   const blockBtn = shadowRoot.querySelector('#blockFriend-btn');
//   const username = shadowRoot.querySelector('.friend-name').textContent;

//   // Determine current action based on button text
//   const isCurrentlyBlocked = blockBtn.textContent === 'Unblock User';

//   if (isCurrentlyBlocked) {
//     // Unblock the user
//     const confirmUnblock = confirm(`Are you sure you want to unblock ${username}?`);
//     if (!confirmUnblock) return; // Exit if user cancels

//     blockBtn.disabled = true; // Prevent multiple clicks

//     const success = await unblockUser(username);
//     if (success) {
//       // Update the Friend Card UI to reflect unblocking
//       const friendshipId = await getFriendshipStatus(username);
//       showFriendCard({ username, image_url: shadowRoot.querySelector('.friend-avatar img').src }, shadowRoot, friendshipId);
//     }

//     blockBtn.disabled = false; // Re-enable button
//   } else {
//     // Block the user
//     const confirmBlock = confirm(`Are you sure you want to block ${username}? This will remove any existing friendship.`);
//     if (!confirmBlock) return; // Exit if user cancels

//     blockBtn.disabled = true; // Prevent multiple clicks

//     const success = await blockUser(username);
//     if (success) {
//       // Update the Friend Card UI to reflect blocking
//       showFriendCard({ username, image_url: shadowRoot.querySelector('.friend-avatar img').src }, shadowRoot, null);
//     }
...
//   // Attach event listener to the "Add Friend" button if not already attached
//   if (!addBtn.dataset.listenerAttached) {
//     addBtn.addEventListener('click', () => handleAddCancelFriend(shadowRoot));
//     addBtn.dataset.listenerAttached = 'true'; // Mark as attached
//   }

//   // Attach event listener to the "Block Friend" / "Unblock User" button if not already attached
//   if (!blockBtn.dataset.listenerAttached) {
//     blockBtn.addEventListener('click', () => handleBlockFriend(shadowRoot));
//     blockBtn.dataset.listenerAttached = 'true'; // Mark as attached
//   }

//   // Attach event listener to the "Close" button if not already attached
//   if (!closeBtn.dataset.listenerAttached) {
//     closeBtn.addEventListener('click', () => hideFriendCard(shadowRoot));
//     closeBtn.dataset.listenerAttached = 'true';
//   }

//   // Attach event listener to the overlay if not already attached
//   if (!overlay.dataset.listenerAttached) {
//     overlay.addEventListener('click', () => hideFriendCard(shadowRoot));
...
//         // Fetch existing friendship status
//         const friendshipId = await getFriendshipStatus(user.username);

//         // Show the Friend Card with the selected user details
//         showFriendCard(user, shadowRoot, friendshipId);
//       });
